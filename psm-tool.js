...
const networks = {
    ...
    ollinet: { rpc: 'https://ollinet.rpc.agoric.net:443', chainId: 'agoricollinet-20' },
};
...
const storageNode = {
    ...
    parseCapData: txt => {
        ...
        const capData = JSON.parse(
            'blockHeight' in specimen ? specimen.values.at(-1) : specimen.value,
        );
        try {
    ...
    unserialize: (txt, ctx) => {
        ...
        return miniMarshal(ctx.convertSlotToVal).unserialze(capData);
    },
    /** @param { string } txt */
    parseEntries: txt => {
...
const makeAgoricNames = async (ctx, getJSON, kinds = ['brand', 'instance']) => {
    const entries = await Promise.all(
        kinds.map(async kind => {
            ...
            const parts = storageNode.unserialize(content, ctx);
...
const getWalletState = async (addr, ctx, { getJSON }) => {
    ...
    const state = storageNode.unserialize(txt, ctx);
    return state;
};
...
const online = async (opts, flags, { fetch }) => {
    const net = networks[opts.net || 'local'] || JSON.parse(opts.net);
    assert(net, opts.net);
    const getJSON = async url => (await fetch(net.rpc + url)).json();
    ...
    if (flags.verbose) {
        ...
        console.error('vstorage published.*', JSON.parse(top).children);
    }
    ...
    if (flags.contract) {
        ...
        const { current: governance } = storageNode.unserialize(
            govContent,
            fromBoard,
        );
        const {
